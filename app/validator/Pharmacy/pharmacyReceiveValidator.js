...
const reviewReceiptSchema = Joi.object({
    administrative: {
        patient_name: Joi.number().min(0).max(1).required(),
        age: Joi.number().min(0).max(1).required(),
        doctor_name: Joi.number().min(0).max(1).required(),
        receipt_date: Joi.number().min(0).max(1).required(),
    },
    pharmaceuticals: {
        drug_name: Joi.number().min(0).max(1).required(),
        strength: Joi.number().min(0).max(1).required(),
        dosage: Joi.number().min(0).max(1).required(),
        ob_total: Joi.number().min(0).max(1).required(),
        rule: Joi.number().min(0).max(1).required(),
    },
    clinic_aspect: {
        true_indication: Joi.number().min(0).max(1).required(),
        true_dosage: Joi.number().min(0).max(1).required(),
        true_time_usage: Joi.number().min(0).max(1).required(),
        drug_duplication: Joi.number().min(0).max(1).required(),
        alergic: Joi.number().min(0).max(1).required(),
        rotd: Joi.number().min(0).max(1).required(),
    },
    created_by: Joi.objectId().required()
})

const pharmacyReceiveValidator = Joi.object({
